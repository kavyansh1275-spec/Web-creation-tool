...
# Public preview mode: generated websites can be opened directly from a link.
preview_name = st.query_params.get("preview")
if preview_name:
    preview_root = (CONFIG.workspace / pm.safe_name(preview_name)).resolve()
    workspace_root = CONFIG.workspace.resolve()
    try:
        preview_root.relative_to(workspace_root)
    except ValueError:
        st.error("Invalid preview project.")
        st.stop()
    index_file = preview_root / "index.html"
    if not index_file.exists():
        st.error("No index.html was found for this project.")
        st.stop()
    html = index_file.read_text(encoding="utf-8", errors="replace")
    css_file = preview_root / "styles.css"
    if css_file.exists() and "styles.css" in html:
        css = css_file.read_text(encoding="utf-8", errors="replace")
        html = html.replace('<link rel="stylesheet" href="styles.css">', "<style>" + css + "</style>")
        html = html.replace('<link rel="stylesheet" href="styles.css"/>', "<style>" + css + "</style>")
    st.title(preview_name)
    st.caption("Generated by Web Creation Tool • Preview mode")
    st.html(html)
    st.stop()
...
def result_message(result):
    if result.get("offline") or result.get("plan", {}).get("offline"):
        return "Offline mode: production generation was not completed."
    if result.get("quality_gate") and not result["quality_gate"].get("passed", True):
        return "Build stopped: quality gate failed."
    return {
...
def show_result(result):
    ...
    if message.startswith("Offline mode") or message.startswith("Build stopped"):
        st.warning(message)
    else:
        st.success(message)
